...
class ISN(nn.Module):
    ...
    def __init__(self, architecture, dim_y, hyperparams):
        ...
        # Alteration
        self.decoder_layer = nn.Linear(architecture[-1], dim_y)
    ...
    def MI(self, z, y, n=10):
        ...
        # [E]. PREDEP 
        if self.estimator == 'PREDEP':
            z = self.encode(z)
            z = self.decoder_layer(z)

            episilon = z - y
            episilon_1 = episilon[torch.randint(0, episilon.shape[0], (1000,))]
            episilon_2 = episilon[torch.randint(0, episilon.shape[0], (1000,))]
